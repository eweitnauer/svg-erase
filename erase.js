		...
		while ( i < (path.length - 1) ) {
			...
			if ( code0 && code1 ) {
				...
				if ( poss_intersects ) {
					...
					// point in the path
					if (new_path[new_path.length-1][0] !== poss_intersects[0][0] ||
              new_path[new_path.length-1][1] !== poss_intersects[0][1]) {
						new_path.push(poss_intersects[0] );
					}

					// we only want paths with length > 1
					if (new_path.length > 1) new_paths.push(new_path);
					...
					if (path[i+1] && path[i+1][0] == poss_intersects[1][0]
                        && path[i+1][1] == poss_intersects[1][1]) i++;
					last = i;
				}
				else {
					i++;
...
function get_circle_intersections (a_x, a_y, b_x, b_y, c_x, c_y, r) {
	...
	*/
	...
	var closest = get_closest_point_on_segment([a_x, a_y], [b_x, b_y], [c_x, c_y]);
	var dist = get_length([closest[0]-c_x, closest[1]-c_y]);
...
var get_closest_point_on_segment = function(A, B, P) {
